 ...
 */
export const scrapeEbay = async (query, maxPages = 3) => {
  ...
  let currentUrl = `https://www.ebay.com/sch/i.html?_nkw=${encodeURIComponent(
    query
  )}`;
  ...
  while (currentPage <= maxPages && currentUrl) {
    ...
    const results = await page.evaluate(() => {
      ...
      for (let el of elements) {
        ...
        const shippingCost = el.querySelector(
          ".s-item__shippingCost"
        )?.innerText; // shippingCost info
...
//             console.log(`shippingCost: ${item.shippingCost}`); // shippingCost info
