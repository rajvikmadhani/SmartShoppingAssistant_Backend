...
export const ebayScraper = async (query, domain, maxPages = 3) => {
  ...
  let currentUrl = `https://www.ebay.${domain}/sch/i.html?_nkw=${encodeURIComponent(
    query
  ...
  while (currentPage <= maxPages && currentUrl) {
    ...
    const results = await page.evaluate(() => {
      ...
      for (let el of elements) {
        ...
        const shippingCostCost = el.querySelector(
          ".s-item__shippingCostCost"
        )?.innerText; // shippingCostCost info
        ...
        if (title && price && link) {
          ...
          items.push({
            ...
            seller: "eBay", // seller name (eBay in this case)
            productSellerRate: 0,
            badge: "Unknown",
            isPrime: false,
...
ebayScraper('iphone', 20)
    .then((scrapedData) => {
        ...
        scrapedData.forEach((item, i) => {
            ...
            console.log(`shippingCostCost: ${item.shippingCostCost}`); // shippingCostCost info
