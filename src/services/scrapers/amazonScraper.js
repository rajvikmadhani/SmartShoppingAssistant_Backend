    ...
    while (currentPage <= maxPages && currentPageUrl) {
        ...
        const results = await page.evaluate(() => {
            ...
            document.querySelectorAll('.s-main-slot .s-result-item').forEach((el) => {
                ...
                const linkEl = el.querySelector('h2 a') || el.querySelector('a.a-link-normal.s-no-outline');
                ...
                const link = linkEl ? `https://www.amazon.com${linkEl.getAttribute('href')}` : undefined; // product link com
                ...
                const delivery = el.querySelector('.a-color-secondary .a-text-bold')?.innerText; // extract the delivery information
...
scrapeAmazon('iphone', 3)
    .then((results) => {
        ...
        results.forEach((item, i) => {
            ...
            console.log(`product seller rate: ${item.productSellerRate}`);
